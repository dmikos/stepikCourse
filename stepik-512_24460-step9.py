...
"""
Ниже в комментариях было сказано, что задача решилась с первого раза при использовании конструкции x = {'global' : {'parent' : None, 'vars' : []}}. Воспользовался сегодня этой конструкцией, переписал весь код и программа заработала.

1. Циклом задаете количество строк, вводимых пользователем
2. Введенную строку разделяете на 3 переменные (условно cmd (которая может принимать значения add, create и get), nsp (пространство имен указанное в строке) и arg (либо переменная для запросов add и get, либо пространство имен родителя для запроса create))
